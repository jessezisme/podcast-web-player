...
export default defineEventHandler(async (event) => {
  ...
  const apiResponse = await $fetch('https://listen-api.listennotes.com/api/v2/best_podcasts', {
    headers: {
  ...
  return new PodcastDataModel().formatBestPodcastsData(apiResponse as BestPodcastTypes.ServerResponseRaw);

  /*
  const finalData = new PodcastDataModel().formatBestPodcastsData({ ...mockBestPodcasts });
  return finalData;
  */
});

const mockBestPodcasts: BestPodcastTypes.ServerResponseRaw = {
  id: 93,
  name: 'Business',
  total: 746,
  has_next: true,
